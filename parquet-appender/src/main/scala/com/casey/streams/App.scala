...
object App {
    def main(args: Array[String]) {
        ...
        val source: String = try {
            ...
            case e: Exception => {
                println("must pass valid arg 1: source path");
        ...
        val output: String = try {
            ...
            case e: Exception => {
                println("must pass valid arg 2: output path");
                throw new Exception("invalid arg2")
            }
        }
        val appendType: String = try {
            args(2).toLowerCase
        } catch {
            case e: Exception => {
                println("must pass valid arg 3: appendType");
                throw new Exception("invalid arg3")
            }
        }
